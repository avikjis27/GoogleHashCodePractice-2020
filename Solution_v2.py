...
if __name__ == '__main__':
    is_inverted_dynamic_programing_applied = False
    max_required_slice, number_of_slices = processInput(sys.argv[1])
    filename = os.path.basename(sys.argv[1])
    total_slices_if_ordered_all_types = np.sum(number_of_slices)
    extra_slices_if_ordered_all_types = total_slices_if_ordered_all_types - max_required_slice
    result = None
    print("total_slices_if_ordered_all_types",total_slices_if_ordered_all_types)
    print("extra_slices_if_ordered_all_types",extra_slices_if_ordered_all_types)
    if (extra_slices_if_ordered_all_types <  max_required_slice):
        is_inverted_dynamic_programing_applied = True
        print(max_required_slice)
        result = calculateRequiredPizzaSlice(extra_slices_if_ordered_all_types, number_of_slices)
    else:
        result = calculateRequiredPizzaSlice(max_required_slice, number_of_slices)
    
    print("Inverted=> ", is_inverted_dynamic_programing_applied)
